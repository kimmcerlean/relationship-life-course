...
mcsa.work<-seqMD(channels=list(seq.work.ow, seq.hw.hrs),
            with.missing=TRUE,
            what="MDseq") ##, right=NA)
...
mcdist.work.om <- seqdistmc(channels=list(seq.work.ow, seq.hw.hrs), ## Seq states NOT om matrix
                           method="OM", 
                           indel=list(work.miss.indel,hw.miss.indel),
                           sm=list(work.miss.cost$sm, hw.miss.cost$sm),
                           with.missing=TRUE) 
