...
// try to create real marital status
tab MARST_DEFACTO_HEAD_ in_sample_, m
tab MARST_DEFACTO_HEAD_ MARST_LEGAL_HEAD_
tab MARST_DEFACTO_HEAD_ rel_type if partnered_==1, m
browse unique_id partner_unique_id survey_yr relationship_ partnered rel_type MARST_DEFACTO_HEAD_ MARST_LEGAL_HEAD_

gen cohab_est_head=0
replace cohab_est_head=1 if MARST_DEFACTO_HEAD_==1 & inlist(MARST_LEGAL_HEAD_,2,3,4,5)
replace cohab_est_head=1 if rel_type==22 & relationship_==1

tab rel_type cohab_est_head if relationship_==1, m

tab MARST_DEFACTO_HEAD_ rel_type if relationship_==1, m

browse unique_id partner_unique_id survey_yr relationship_ partnered rel_type cohab_est_head MARST_DEFACTO_HEAD_ MARST_LEGAL_HEAD_

gen marital_status_head =.
replace marital_status_head=1 if MARST_DEFACTO_HEAD_==1 & cohab_est_head==0
replace marital_status_head=2 if MARST_DEFACTO_HEAD_==1 & cohab_est_head==1
...
label define marital_status_updated 1 "Married" 2 "Partnered" 3 "Never Married" 4 "Widowed" 5 "Divorced" 6 "Separated"
label values marital_status_head marital_status_updated

tab marital_status_head rel_type if partnered==1, m
tab marital_status_head rel_type if relationship_==1 & partnered==1, m
tab marital_status_head rel_type if relationship_==2 & partnered==1, m
...
// Marital Status
label values NUM_MARRIED .
// browse unique_id partner_unique_id survey_yr first_survey_year last_survey_year relationship_ partnered rel_type NUM_MARRIED MARITAL_STATUS marital_status_head FIRST_MARRIAGE_YR_START FIRST_MARRIAGE_YR_END FIRST_MARRIAGE_STATUS  RECENT_MARRIAGE_YR_START RECENT_MARRIAGE_YR_END RECENT_MARRIAGE_STATUS
tab marital_status_head rel_type if relationship_==1, m
tab marital_status_head rel_type if relationship_==2, m

gen ever_married = .
replace ever_married = 0 if NUM_MARRIED==0
replace ever_married = 1 if inrange(NUM_MARRIED,1,10)

// tab FIRST_MARRIAGE_YR_END FIRST_MARRIAGE_STATUS, m // 1 = intact, 3 = widowhood, 4 = divorce, 5 = separated
// tab FIRST_SEPARATE_YR FIRST_MARRIAGE_STATUS, m
tab MARITAL_STATUS if survey_yr >= FIRST_MARRIAGE_YR_END & ever_married==1 & FIRST_MARRIAGE_YR_END < 9000, m
tab FIRST_MARRIAGE_STATUS MARITAL_STATUS if survey_yr >= FIRST_MARRIAGE_YR_END & NUM_MARRIED==1 & FIRST_MARRIAGE_YR_END < 9000, m
tab FIRST_MARRIAGE_STATUS MARITAL_STATUS if survey_yr >= FIRST_SEPARATE_YR & NUM_MARRIED==1 & FIRST_SEPARATE_YR < 9000, m

// check IDs: 2464174

gen marital_status_focal=.
replace marital_status_focal = marital_status_head if relationship_==1
replace marital_status_focal = marital_status_head if relationship_==2 // should apply to partner as well
replace marital_status_focal = 1 if marital_status_focal==. & rel_type == 20
replace marital_status_focal = 2 if marital_status_focal==. & rel_type == 22
replace marital_status_focal = 3 if marital_status_focal==. & MARITAL_STATUS == 2 & survey_yr <= last_survey_year // if LAST KNOWN marital status is never married and not currently partnered, then we can assume never married at survey year (because can't switch from divorced back). this is probably only case where this is true.
replace marital_status_focal = 3 if marital_status_focal==. & survey_yr < FIRST_MARRIAGE_YR_START & FIRST_MARRIAGE_YR_START!=. & FIRST_MARRIAGE_YR_START!=9999 & FIRST_MARRIAGE_YR_START!=9998 // if not currently partnered and before first marriage year (if have a first marriage year), can also assume never married
replace marital_status_focal = 3 if marital_status_focal==. & partnered==0 & NUM_MARRIED==0 & survey_yr <= last_survey_year // if no marriages and not partnered, then never married. I think this actually covered with above

replace marital_status_focal = 4 if marital_status_focal==. & partnered==0 & NUM_MARRIED==1 & FIRST_MARRIAGE_STATUS==3 & survey_yr >= FIRST_MARRIAGE_YR_END & FIRST_MARRIAGE_YR_END < 9000 & survey_yr <= last_survey_year // if only married once and currently unpartnered, can use status of first marriage if not partnered? so this one is widowhood
replace marital_status_focal = 5 if marital_status_focal==. & partnered==0 & NUM_MARRIED==1 & FIRST_MARRIAGE_STATUS==4 & survey_yr >= FIRST_MARRIAGE_YR_END & FIRST_MARRIAGE_YR_END < 9000 & survey_yr <= last_survey_year // this one is divorce
replace marital_status_focal = 5 if marital_status_focal==. & partnered==0 & NUM_MARRIED==1 & FIRST_MARRIAGE_STATUS==4 & survey_yr >= FIRST_SEPARATE_YR & FIRST_SEPARATE_YR < 9000 & survey_yr <= last_survey_year  // alt divorce
replace marital_status_focal = 6 if marital_status_focal==. & partnered==0 & NUM_MARRIED==1 & FIRST_MARRIAGE_STATUS==5 & survey_yr >= FIRST_SEPARATE_YR & FIRST_SEPARATE_YR < 9000 & survey_yr <= last_survey_year // this one is separation

label values marital_status_focal marital_status_updated
tab marital_status_focal in_sample_, m col
tab marital_status_focal rel_type if partnered==1, m
tab marital_status_focal partnered, m

browse unique_id partner_unique_id survey_yr relationship_ partnered rel_type marital_status_focal NUM_MARRIED MARITAL_STATUS marital_status_head FIRST_MARRIAGE_YR_START FIRST_MARRIAGE_YR_END FIRST_MARRIAGE_STATUS  RECENT_MARRIAGE_YR_START RECENT_MARRIAGE_YR_END RECENT_MARRIAGE_STATUS
