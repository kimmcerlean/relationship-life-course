...
## HW, let's explore several variables for now:
  ...
  # couple_hw_hrs_weekly_trunc (Core HW, weekday * 5 + sat + sun)
  # couple_hw_hrs_combined_trunc (Core HW + repairs, weekday)
## family_type_trunc:	Type of family based on relationship type + number of children
...
col_hw.hrs.weekly =which(colnames(data)%in%lab_t) 
...
# Couple HW - Weekly
seq.hw.hrs.weekly <- seqdef(data[,col_hw.hrs.weekly], cpal = colspace.hw.hrs.combo, labels=longlab.hw.hrs.combo, 
                             states= shortlab.hw.hrs.combo,right=NA)

seq.len.hw.weekly<-seqlength(seq.hw.hrs.weekly, with.missing = FALSE)

ggseqdplot(seq.hw.hrs.weekly) +
  scale_x_discrete(labels = 1:10) +
  labs(x = "Year")
...
s2b<-ggseqdplot(seq.hw.hrs.weekly) +
  scale_x_discrete(labels = 1:10) +
  labs(x = "Relationship Duration", y=NULL) + 
  theme(legend.position="none") +
...
seqtrate(seq.hw.hrs.weekly)
seqtrate(seq.fam)

work.trate.subcosts <- seqsubm(seq.work.ow, method = "TRATE")
hw.trate.subcosts <- seqsubm(seq.hw.hrs.weekly, method = "TRATE")
