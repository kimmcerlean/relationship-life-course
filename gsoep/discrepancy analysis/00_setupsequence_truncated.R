...
data <- read_dta("temp data/gsoep/gsoep_couples_wide_truncated_tmp.dta")
# data <- data%>%filter(`_mi_m`!=0)
data <- data%>%filter(`_mi_m`==1) # test with 1 until I sort this out.
...
table(data$couple_educ_type) # ensure variable here
table(data$one_college) # ensure variable here
...
## couple_hw_hrs_weekly_trunc:	Couple-level housework indicator, split by time spent on HW for some categories (consolidated)
  # for Germany, this is the weekly estimated (Core HW, weekday * 5 + sat + sun)
