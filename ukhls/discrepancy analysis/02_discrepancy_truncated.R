...
load("G:/Other computers/My Laptop/Documents/Research Projects/Relationship Life Course (with LP)/discrepancy analysis exploration/ukhls-setupsequence-truncated.RData")
...
seqIplot(seq.fam, group = data$one_college,  sortv = "from.start", with.missing = FALSE) 
seqIplot(seq.work.ow, group = data$one_college, sortv = "from.start", with.missing = FALSE) 
...
pdf("G:/Other computers/My Laptop/Documents/Research Projects/Relationship Life Course (with LP)/discrepancy analysis exploration/UKHLS_MCIndex_4Groups.pdf",
    width=8,
    height=11)
...
seqdplot(seq.work.ow, group = data$couple_educ_type) # okay the UK does seem more like the uS here in terms of diffs? and him college is really still similar to neither
...
#I actually really like these. okay the UK brings me back to thinking this will work
# (reflect also on like - is it market-oriented / liberal regimes that exacerbate educ diffs? MAYBE)
# EXCEPT family - family is MOST distinct in US - also the bar to marriage is primarily a thing in uS? okay i think this is the paper I want to write next, just need to think about framing...
seqdplot(seq.fam, group = data$one_college, yaxis=FALSE, xaxis=FALSE) # weirdly MORE cohab at start among more educated? (and more kids among less) is this odd?
seqdplot(seq.work.ow, group = data$one_college, yaxis=FALSE, xaxis=FALSE) # okay yeah this is really like US trends and I think IS so interesting. education differences CLOSE when people stay in sample long term AND gender egal declines
...
write_xlsx(combined_results, "G:/Other computers/My Laptop/Documents/Research Projects/Relationship Life Course (with LP)/discrepancy analysis exploration/UKHLS_BIC_LRT_tests.xlsx")
...
#         out='csv', file="G:/Other computers/My Laptop/Documents/Research Projects/Relationship Life Course (with LP)/discrepancy analysis exploration/ukhls_educ_desctable_truncated.csv")
...
sumtable(educ_metric_table, digits = 4, numformat = NA, group = 'couple_educ_type', group.test = TRUE, 
         out='csv', file="G:/Other computers/My Laptop/Documents/Research Projects/Relationship Life Course (with LP)/discrepancy analysis exploration/ukhls_educ_metrics_truncated.csv")
