...
# load("created data/ukhls/ukhls-setupsequence-truncated_mi1.RData") # to run temp on my computer (non-cluster specific)

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Add cluster information to source data ---- 
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# We are using 8 clusters for now
...
desctable <- subset(data,
                    select = c(mc8.factor, fam_lgth, fam_lgth_nomiss, fam_spells,
...
desctable$clusters <- as.character(desctable$mc8.factor)

sumtable(desctable, digits = 4, fixed.digits = TRUE, numformat = NA, group = 'clusters', group.test = TRUE, 
         out='csv', file='results/UKHLS/tables/cluster_desctable_truncated.csv')
...
state.time.fam.c1 <- seqmeant(mc8.fam1.seq)
state.time.fam.c2 <- seqmeant(mc8.fam2.seq)
state.time.fam.c3 <- seqmeant(mc8.fam3.seq)
state.time.fam.c4 <- seqmeant(mc8.fam4.seq)
state.time.fam.c5 <- seqmeant(mc8.fam5.seq)
state.time.fam.c6 <- seqmeant(mc8.fam6.seq)
state.time.fam.c7 <- seqmeant(mc8.fam7.seq)
state.time.fam.c8 <- seqmeant(mc8.fam8.seq)
...
write.xlsx(state.time.family, "results/UKHLS/tables/ukhls_family_statedurs.xlsx")

# Work
state.time.work.ow <- seqmeant(seq.work.ow)
state.time.work.ow.c1 <- seqmeant(mc8.work.ow1.seq)
state.time.work.ow.c2 <- seqmeant(mc8.work.ow2.seq)
state.time.work.ow.c3 <- seqmeant(mc8.work.ow3.seq)
state.time.work.ow.c4 <- seqmeant(mc8.work.ow4.seq)
state.time.work.ow.c5 <- seqmeant(mc8.work.ow5.seq)
state.time.work.ow.c6 <- seqmeant(mc8.work.ow6.seq)
state.time.work.ow.c7 <- seqmeant(mc8.work.ow7.seq)
state.time.work.ow.c8 <- seqmeant(mc8.work.ow8.seq)
...
write.xlsx(state.time.work, "results/UKHLS/tables/ukhls_work_statedurs.xlsx")


# Housework
state.time.hw.hrs <- seqmeant(seq.hw.hrs)
state.time.hw.hrs.c1 <- seqmeant(mc8.hw.hrs1.seq)
state.time.hw.hrs.c2 <- seqmeant(mc8.hw.hrs2.seq)
state.time.hw.hrs.c3 <- seqmeant(mc8.hw.hrs3.seq)
state.time.hw.hrs.c4 <- seqmeant(mc8.hw.hrs4.seq)
state.time.hw.hrs.c5 <- seqmeant(mc8.hw.hrs5.seq)
state.time.hw.hrs.c6 <- seqmeant(mc8.hw.hrs6.seq)
state.time.hw.hrs.c7 <- seqmeant(mc8.hw.hrs7.seq)
state.time.hw.hrs.c8 <- seqmeant(mc8.hw.hrs8.seq)
...
write.xlsx(state.time.hw, "results/UKHLS/tables/ukhls_hw_statedurs.xlsx")
...
# removing this code for now (if needed, can find in complete cluster folder)
...
# Wanted to just explore the differences
#test_work_deg <- subset(data,
#       select = c(couple_work_ow_trunc1, couple_work_ow_trunc2, couple_work_ow_trunc3,
#                  couple_work_ow_trunc4, couple_work_ow_trunc5, couple_work_ow_trunc6,
#                  couple_work_ow_trunc7, couple_work_ow_trunc8, couple_work_ow_trunc9,
#                  couple_work_ow_trunc10, work_degrad_neg, work_degrad_pos,
#                  work_degrad_both))

#write.csv(test_work_deg, 'results/UKHLS/tables/UKHLS_test_work_degrad.csv')
...
domain_table <- subset(data,
                    select = c(mc8.factor, time_marriage, time_child, time_child.mar,
...
# oh, this is just a new column
domain_table$clusters <- as.character(domain_table$mc8.factor)

sumtable(domain_table, digits = 4, numformat = NA, group = 'clusters', group.test = TRUE, 
         out='csv', file='results/UKHLS/tables/cluster_domain_info.csv')
...
# Save
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
save.image("created data/ukhls/UKHLS_trunc_cluster_descriptives.RData")
